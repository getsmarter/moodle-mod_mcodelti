...
/**
 * Exception handler for mcodelti services
 *
...
/**
 * Handles exceptions when handling incoming mcodelti messages.
 *
 * Ensures that mcodelti always returns a XML message that can be consumed by the caller.
 *
 * @package   mod_mcodelti
 * @copyright Copyright (c) 2015 Moodlerooms Inc. (http://www.moodlerooms.com)
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class service_exception_handler {
    ...
    /**
     * The mcodelti service message ID, if known.
     *
     * @var string
     */
    protected $id = '';
    ...
    /**
     * The mcodelti service message type, if known.
     *
     * @var string
     */
    ...
    /**
     * Set the mcodelti message ID being handled.
     *
     * @param string $id
     */
    ...
    /**
     * Set the mcodelti message type being handled.
     *
     * @param string $type
     */
    ...
    public function handle($exception) {
        ...
        $response = mcodelti_get_response_xml('failure', $message, $this->id, $type);

        $xml = $response->asXML();

        // Log the request if necessary.
        if ($this->log) {
            mcodelti_log_response($xml, $exception);
        }
