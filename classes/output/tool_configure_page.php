...
class tool_configure_page implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $keyhelp = new help_icon('resourcekey', 'mod_mcodelti');
        $secrethelp = new help_icon('password', 'mod_mcodelti');

        $url = new moodle_url('/mod/mcodelti/typessettings.php', array('sesskey' => sesskey(), 'returnto' => 'toolconfigure'));
        $data->configuremanualurl = $url->out();
        $url = new moodle_url('/admin/settings.php?section=modsettingmcodelti');
        $data->managetoolsurl = $url->out();
        $url = new moodle_url('/mod/mcodelti/toolproxies.php');
