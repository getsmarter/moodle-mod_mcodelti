...
/**
 * This file contains an abstract definition of an mcodelti service
 *
...
namespace mod_mcodelti\local\mcodeltiservice;
...
/**
 * The mod_mcodelti\local\mcodeltiservice\service_base class.
 *
 * @package    mod_mcodelti
 * @since      Moodle 2.8
 * @copyright  2014 Vital Source Technologies http://vitalsource.com
...
abstract class service_base {
    ...
    /** Label representing an mcodelti 2 message type */
    const mcodelti_VERSION2P0 = 'mcodelti-2p0';
    ...
    public static function get_service_path() {
        ...
        $url = new \moodle_url('/mod/mcodelti/services.php');
    ...
    public function check_tool_proxy($toolproxyguid, $body = null) {
        ...
        $ok = false;
        $toolproxy = null;
        $consumerkey = mcodelti\get_oauth_key_from_headers();
        if (empty($toolproxyguid)) {
            $toolproxyguid = $consumerkey;
        }

        if (!empty($toolproxyguid)) {
            $toolproxy = mcodelti_get_tool_proxy_from_guid($toolproxyguid);
            if ($toolproxy !== false) {
    ...
    private function check_signature($consumerkey, $secret, $body) {
        ...
        try {
            // TODO: Switch to core oauthlib once implemented - MDL-30149.
            mcodelti\handle_oauth_body_post($consumerkey, $secret, $body);
        } catch (\Exception $e) {
