...
$messagetype = required_param('mcodelti_message_type', PARAM_TEXT);
$version = required_param('mcodelti_version', PARAM_TEXT);
...
require_capability('mod/mcodelti:addcoursetool', $context);

$redirecturl = null;
$returndata = null;
if (empty($errormsg) && !empty($items)) {
    try {
        $returndata = mcodelti_tool_configuration_from_content_item($id, $messagetype, $version, $consumerkey, $items);
    } catch (moodle_exception $e) {
        $errormsg = $e->getMessage();
    }
}

$pageurl = new moodle_url('/mod/mcodelti/contentitem_return.php');
$PAGE->set_url($pageurl);
$PAGE->set_pagelayout('popup');
echo $OUTPUT->header();

// Call JS module to redirect the user to the course page or close the dialogue on error/cancel.
$PAGE->requires->js_call_amd('mod_mcodelti/contentitem_return', 'init', [$returndata]);
...
if ($errormsg) {
    ...
    // Means success.
    if (!$msg) {
        $msg = get_string('successfullyfetchedtoolconfigurationfromcontent', 'mcodelti');
    }
