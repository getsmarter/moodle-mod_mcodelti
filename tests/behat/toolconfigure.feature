...
Feature: Configure tool types
  In order to allow teachers to add external mcodelti tools
  As an admin
  ...
  Scenario: Add a tool type from a cartridge URL
    When I set the field "url" to local url "/mod/mcodelti/tests/fixtures/ims_cartridge_basic_mcodelti_link.xml"
    And I press "Add"
  ...
  Scenario: Try to add a non-existant cartridge
    When I set the field "url" to local url "/mod/mcodelti/tests/fixtures/nonexistant.xml"
    And I press "Add"
    Then I should see "Enter your consumer key and shared secret"
    And I press "Save changes"
  ...
  Scenario: Attempt to add a tool type from a configuration URL, then cancel
    When I set the field "url" to local url "/mod/mcodelti/tests/fixtures/tool_provider.php"
    And I press "Add"
