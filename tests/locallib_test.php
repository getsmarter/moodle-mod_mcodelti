...
/**
 * This file contains unit tests for (some of) mcodelti/locallib.php
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_mcodelti_locallib_testcase extends advanced_testcase {
    ...
    public function test_split_custom_parameters() {
        ...
        $tool->parameter = '';
        $this->assertEquals(mcodelti_split_custom_parameters(null, $tool, array(), "x=1\ny=2", false),
            array('custom_x' => '1', 'custom_y' => '2'));

        // Check params with caps.
        $this->assertEquals(mcodelti_split_custom_parameters(null, $tool, array(), "X=1", false),
            array('custom_x' => '1', 'custom_X' => '1'));

        // Removed repeat of previous test with a semicolon separator.

        $this->assertEquals(mcodelti_split_custom_parameters(null, $tool, array(), 'Review:Chapter=1.2.56', false),
            array('custom_review_chapter' => '1.2.56', 'custom_Review_Chapter' => '1.2.56'));

        $this->assertEquals(mcodelti_split_custom_parameters(null, $tool, array(),
        ...
        $this->assertEquals(array('custom_x' => '1', 'custom_y' => 'SOMETHING'),
            mcodelti_split_custom_parameters(null, $tool, array(), "x=1\ny=\$Person.name.middle", false));
    }
    ...
    public function disabled_test_sign_parameters() {
        $correct = array ( 'context_id' => '12345', 'context_label' => 'SI124', 'context_title' => 'Social Computing',
            'ext_submit' => 'Click Me', 'mcodelti_message_type' => 'basic-mcodelti-launch-request', 'mcodelti_version' => 'mcodelti-1p0',
        ...
        $parms = mcodelti_sign_parameters($requestparams, 'http://www.imsglobal.org/developer/mcodelti/tool.php', 'POST',
    ...
    public function disabled_test_parse_grade_replace_message() {
        $message = '
            <imsx_POXEnvelopeRequest xmlns = "http://www.imsglobal.org/services/mcodeltiv1p1/xsd/imsoms_v1p0">
              <imsx_POXHeader>
...
';
        ...
        $parsed = mcodelti_parse_grade_replace_message(new SimpleXMLElement($message));
        ...
        $mcodeltiinstance = (object)array('servicesalt' => '4e5fcc06de1d58.44963230');

        mcodelti_verify_sourcedid($mcodeltiinstance, $parsed);
    }

    public function test_mcodelti_ensure_url_is_https() {
        $this->assertEquals('https://moodle.org', mcodelti_ensure_url_is_https('http://moodle.org'));
        $this->assertEquals('https://moodle.org', mcodelti_ensure_url_is_https('moodle.org'));
        $this->assertEquals('https://moodle.org', mcodelti_ensure_url_is_https('https://moodle.org'));
    }

    /**
     * Test mcodelti_get_url_thumbprint against various URLs
     */
    public function test_mcodelti_get_url_thumbprint() {
        // Note: trailing and double slash are expected right now.  Must evaluate if it must be removed at some point.
        $this->assertEquals('moodle.org/', mcodelti_get_url_thumbprint('http://MOODLE.ORG'));
        $this->assertEquals('moodle.org/', mcodelti_get_url_thumbprint('http://www.moodle.org'));
        $this->assertEquals('moodle.org/', mcodelti_get_url_thumbprint('https://www.moodle.org'));
        $this->assertEquals('moodle.org/', mcodelti_get_url_thumbprint('moodle.org'));
        $this->assertEquals('moodle.org//this/is/moodle', mcodelti_get_url_thumbprint('http://moodle.org/this/is/moodle'));
        $this->assertEquals('moodle.org//this/is/moodle', mcodelti_get_url_thumbprint('https://moodle.org/this/is/moodle'));
        $this->assertEquals('moodle.org//this/is/moodle', mcodelti_get_url_thumbprint('moodle.org/this/is/moodle'));
        $this->assertEquals('moodle.org//this/is/moodle', mcodelti_get_url_thumbprint('moodle.org/this/is/moodle?'));
        $this->assertEquals('moodle.org//this/is/moodle?foo=bar', mcodelti_get_url_thumbprint('moodle.org/this/is/moodle?foo=bar'));
    }

    /*
     * Verify that mcodelti_build_request does handle resource_link_id as expected
     */
    public function test_mcodelti_buid_request_resource_link_id() {
        $this->resetAfterTest();

        self::setUser($this->getDataGenerator()->create_user());
        $course   = $this->getDataGenerator()->create_course();
        $instance = $this->getDataGenerator()->create_module('mcodelti', array(
            'intro'       => "<p>This</p>\nhas\r\n<p>some</p>\nnew\n\rlines",
        ...
        $params = mcodelti_build_request($instance, $typeconfig, $course, null);
    ...
    /**
     * Test mcodelti_build_request's resource_link_description and ensure
     * that the newlines in the description are correct.
     */
    public function test_mcodelti_build_request_description() {
        $this->resetAfterTest();
    ...
    /**
     * Tests mcodelti_prepare_type_for_save's handling of the "Force SSL" configuration.
     */
    public function test_mcodelti_prepare_type_for_save_forcessl() {
        $type = new stdClass();
        $config = new stdClass();

        // Try when the forcessl config property is not set.
        mcodelti_prepare_type_for_save($type, $config);
        $this->assertObjectHasAttribute('mcodelti_forcessl', $config);
        $this->assertEquals(0, $config->mcodelti_forcessl);
        $this->assertEquals(0, $type->forcessl);

        // Try when forcessl config property is set.
        $config->mcodelti_forcessl = 1;
        mcodelti_prepare_type_for_save($type, $config);
        $this->assertObjectHasAttribute('mcodelti_forcessl', $config);
        $this->assertEquals(1, $config->mcodelti_forcessl);
        $this->assertEquals(1, $type->forcessl);

        // Try when forcessl config property is set to 0.
        $config->mcodelti_forcessl = 0;
        mcodelti_prepare_type_for_save($type, $config);
        $this->assertObjectHasAttribute('mcodelti_forcessl', $config);
        $this->assertEquals(0, $config->mcodelti_forcessl);
        $this->assertEquals(0, $type->forcessl);
    ...
    /**
     * Tests mcodelti_load_type_from_cartridge and mcodelti_load_type_if_cartridge
     */
    public function test_mcodelti_load_type_from_cartridge() {
        $type = new stdClass();
        $type->mcodelti_toolurl = $this->getExternalTestFileUrl('/ims_cartridge_basic_mcodelti_link.xml');

        mcodelti_load_type_if_cartridge($type);

        $this->assertEquals('Example tool', $type->mcodelti_typename);
        $this->assertEquals('Example tool description', $type->mcodelti_description);
        $this->assertEquals('http://www.example.com/mcodelti/provider.php', $type->mcodelti_toolurl);
        $this->assertEquals('http://download.moodle.org/unittest/test.jpg', $type->mcodelti_icon);
        $this->assertEquals('https://download.moodle.org/unittest/test.jpg', $type->mcodelti_secureicon);
    }

    /**
     * Tests mcodelti_load_tool_from_cartridge and mcodelti_load_tool_if_cartridge
     */
    public function test_mcodelti_load_tool_from_cartridge() {
        $mcodelti = new stdClass();
        $mcodelti->toolurl = $this->getExternalTestFileUrl('/ims_cartridge_basic_mcodelti_link.xml');

        mcodelti_load_tool_if_cartridge($mcodelti);

        $this->assertEquals('Example tool', $mcodelti->name);
        $this->assertEquals('Example tool description', $mcodelti->intro);
        $this->assertEquals('http://www.example.com/mcodelti/provider.php', $mcodelti->toolurl);
        $this->assertEquals('https://www.example.com/mcodelti/provider.php', $mcodelti->securetoolurl);
        $this->assertEquals('http://download.moodle.org/unittest/test.jpg', $mcodelti->icon);
        $this->assertEquals('https://download.moodle.org/unittest/test.jpg', $mcodelti->secureicon);
    }

    /**
     * Tests for mcodelti_build_content_item_selection_request().
     */
    public function test_mcodelti_build_content_item_selection_request() {
        $this->resetAfterTest();
        ...
        $type = new stdClass();
        $data = new stdClass();
        $data->mcodelti_contentitem = true;
        ...
        // Default parameters.
        $result = mcodelti_build_content_item_selection_request($typeid, $course, $returnurl);
        ...
        $this->assertEquals('ContentItemSelectionRequest', $params['mcodelti_message_type']);
        $this->assertEquals(mcodelti_VERSION_2, $params['mcodelti_version']);
        $this->assertEquals('application/vnd.ims.mcodelti.v1.mcodeltilink', $params['accept_media_types']);
        ...
        $this->assertEquals('false', $params['accept_mumcodeltiple']);
        ...
        $result = mcodelti_build_content_item_selection_request($typeid, $course, $returnurl, $title, $text, $mediatypes, $targets,
        ...
        $this->assertEquals('true', $params['accept_mumcodeltiple']);
    ...
    /**
     * Test for mcodelti_build_content_item_selection_request() with nonexistent tool type ID parameter.
     */
    public function test_mcodelti_build_content_item_selection_request_invalid_tooltype() {
        $this->resetAfterTest();
        ...
        mcodelti_build_content_item_selection_request(1, $course, $returnurl);
    }

    /**
     * Test for mcodelti_build_content_item_selection_request() with invalid media types parameter.
     */
    public function test_mcodelti_build_content_item_selection_request_invalid_mediatypes() {
        $this->resetAfterTest();
        ...
        mcodelti_build_content_item_selection_request($typeid, $course, $returnurl, '', '', $mediatypes);
    }

    /**
     * Test for mcodelti_build_content_item_selection_request() with invalid presentation targets parameter.
     */
    public function test_mcodelti_build_content_item_selection_request_invalid_presentationtargets() {
        $this->resetAfterTest();
        ...
        $this->expectException('coding_exception');
        mcodelti_build_content_item_selection_request($typeid, $course, $returnurl, '', '', [], $targets);
    }

    /**
     * Provider for test_mcodelti_get_best_tool_by_url.
     *
     * @return array of [urlToTest, expectedTool, allTools]
     */
    public function mcodelti_get_best_tool_by_url_provider() {
        $tools = [
            (object) [
                'name' => 'Here',
                'baseurl' => 'https://example.com/i/am/?where=here',
                'tooldomain' => 'example.com',
                'state' => mcodelti_TOOL_STATE_CONFIGURED,
                'course' => SITEID
            ],
            (object) [
                'name' => 'There',
    ...
    /**
     * Test mcodelti_get_best_tool_by_url.
     *
     * @dataProvider mcodelti_get_best_tool_by_url_provider
     * @param string $url The URL to test.
     * @param object $expected The expected tool matching the URL.
     * @param array $tools The pool of tools to match the URL with.
     */
    public function test_mcodelti_get_best_tool_by_url($url, $expected, $tools) {
        $actual = mcodelti_get_best_tool_by_url($url, $tools, null);
