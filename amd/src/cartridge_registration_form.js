 ...
 *
 * See template: mod_mcodelti/cartridge_registration_form
 *
 * @module     mod_mcodelti/cartridge_registration_form
 ...
 * @since      3.1
 */
define(['jquery', 'core/ajax', 'core/notification', 'mod_mcodelti/tool_type', 'mod_mcodelti/events', 'mod_mcodelti/keys', 'core/str'],
        function($, ajax, notification, toolType, mcodeltiEvents, KEYS, str) {
    ...
    var submitCartridgeURL = function() {
        ...
        promise.done(function() {
            str.get_string('successfullycreatedtooltype', 'mod_mcodelti').done(function(s) {
                ...
                $(document).trigger(mcodeltiEvents.STOP_CARTRIDGE_REGISTRATION);
                $(document).trigger(mcodeltiEvents.REGISTRATION_FEEDBACK, {
                    message: s
                });
            }).fail(notification.exception);
        }).fail(function() {
            str.get_string('failedtocreatetooltype', 'mod_mcodelti').done(function(s) {
                $(document).trigger(mcodeltiEvents.NEW_TOOL_TYPE);
                $(document).trigger(mcodeltiEvents.STOP_CARTRIDGE_REGISTRATION);
                $(document).trigger(mcodeltiEvents.REGISTRATION_FEEDBACK, {
                    message: s,
