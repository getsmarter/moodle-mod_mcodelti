 ...
 *
 * See template: mod_mcodelti/external_registration
 *
 * @module     mod_mcodelti/external_registration
 ...
 * @since      3.1
 */
define(['jquery', 'core/ajax', 'core/notification', 'core/templates', 'mod_mcodelti/events',
        'mod_mcodelti/tool_proxy', 'mod_mcodelti/tool_type', 'mod_mcodelti/keys', 'core/str'],
        function($, ajax, notification, templates, mcodeltiEvents, toolProxy, toolType, KEYS, str) {
     ...
     *
     * See mod_mcodelti/tool_proxy_registration_form template.
     *
    ...
    var getRegistrationRequest = function(id) {
        var request = {
            methodname: 'mod_mcodelti_get_tool_proxy_registration_request',
            args: {
                id: id
            }
        };
    ...
    var cancelRegistration = function() {
        ...
        promise.done(function() {
            ...
            finishExternalRegistration();
            stopLoadingCancel();
            str.get_string('failedtodeletetoolproxy', 'mod_mcodelti').done(function(s) {
                var feedback = {
                    message: s,
                    error: true
                };
                $(document).trigger(mcodeltiEvents.REGISTRATION_FEEDBACK, feedback);
    ...
    var renderExternalRegistrationWindow = function(registrationRequest) {
        var promise = templates.render('mod_mcodelti/tool_proxy_registration_form', registrationRequest);
    ...
    var promptForToolTypeCapabilitiesAgreement = function(typeData) {
        ...
        templates.render('mod_mcodelti/tool_type_capabilities_agree', typeData).done(function(html, js) {
            ...
            choiceContainer.on(mcodeltiEvents.CAPABILITIES_AGREE, function() {
            ...
            // wish.
            choiceContainer.on(mcodeltiEvents.CAPABILITIES_DECLINE, function() {
                container.empty();
                promise.resolve();
            });
        }).fail(promise.reject);
    ...
    var finishExternalRegistration = function() {
        ...
        $(document).trigger(mcodeltiEvents.STOP_EXTERNAL_REGISTRATION);
    };
    ...
    var registerEventListeners = function() {
        ...
        $(document).on(mcodeltiEvents.START_EXTERNAL_REGISTRATION, function(event, data) {
                if (!data) {
                    return;
                }
                if (data.url) {
        ...
        // parameters.
        //
        // See AMD module mod_mcodelti/external_registration_return.
        window.triggerExternalRegistrationComplete = function(data) {
            ...
            if (data.status == "success") {
                str.get_string('successfullycreatedtooltype', 'mod_mcodelti').done(function(s) {
                    feedback.message = s;
                }).fail(notification.exception);
                ...
                promise.done(function() {
                    finishExternalRegistration();
                    $(document).trigger(mcodeltiEvents.REGISTRATION_FEEDBACK, feedback);
                    $(document).trigger(mcodeltiEvents.NEW_TOOL_TYPE);
                }).fail(notification.exception);
    ...
    return /** @alias module:mod_mcodelti/external_registration */ {
