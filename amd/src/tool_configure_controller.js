 ...
 * In addition, it can batch mumcodeltiple requests and return mumcodeltiple responses.
 *
 * @module     mod_mcodelti/tool_configure_controller
 ...
 * @since      3.1
 */
define(['jquery', 'core/ajax', 'core/notification', 'core/templates', 'mod_mcodelti/events', 'mod_mcodelti/keys', 'mod_mcodelti/tool_type',
        'mod_mcodelti/tool_proxy', 'core/str'],
        function($, ajax, notification, templates, mcodeltiEvents, KEYS, toolType, toolProxy, str) {
    ...
    var reloadToolList = function() {
        ...
        $.when(
            ...
            .done(function(types, proxies) {
                    templates.render('mod_mcodelti/tool_list', {tools: types, proxies: proxies})
    ...
    var addTool = function() {
        ...
        promise.done(function(result) {
            if (result.iscartridge) {
                ...
                $(document).trigger(mcodeltiEvents.START_CARTRIDGE_REGISTRATION, url);
            } else {
                $(document).trigger(mcodeltiEvents.START_EXTERNAL_REGISTRATION, {url: url});
            }
        });

        promise.fail(function() {
            str.get_string('errorbadurl', 'mod_mcodelti')
                .done(function(s) {
                        $(document).trigger(mcodeltiEvents.REGISTRATION_FEEDBACK, {
                                message: s,
                                error: true
                            });
                    })
    ...
    var registerEventListeners = function() {
        ...
        $(document).on(mcodeltiEvents.NEW_TOOL_TYPE, function() {
            reloadToolList();
        });

        $(document).on(mcodeltiEvents.START_EXTERNAL_REGISTRATION, function() {
            showExternalRegistration();
            $(SELECTORS.TOOL_URL).val('');
            hideToolList();
        });

        $(document).on(mcodeltiEvents.STOP_EXTERNAL_REGISTRATION, function() {
            showToolList();
            showRegistrationChoices();
        });

        $(document).on(mcodeltiEvents.START_CARTRIDGE_REGISTRATION, function(event, url) {
            showCartridgeRegistration(url);
        });

        $(document).on(mcodeltiEvents.STOP_CARTRIDGE_REGISTRATION, function() {
            getCartridgeRegistrationContainer().find(SELECTORS.CARTRIDGE_REGISTRATION_FORM).removeAttr('data-cartridge-url');
            showRegistrationChoices();
        });

        $(document).on(mcodeltiEvents.REGISTRATION_FEEDBACK, function(event, data) {
    ...
    return /** @alias module:mod_mcodelti/cartridge_registration_form */ {
