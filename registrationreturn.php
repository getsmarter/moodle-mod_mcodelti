...
if (empty($top)) {
    ...
    $params['top'] = '1';
    if (!empty($msg)) {
        $params['mcodelti_msg'] = $msg;
    }
    if (!empty($err)) {
        $params['mcodelti_errormsg'] = $err;
    }
    if (!empty($id)) {
        $params['id'] = $id;
    }
    $redirect = new moodle_url('/mod/mcodelti/registrationreturn.php', $params);
    $redirect = $redirect->out(false);

    $clickhere = get_string('click_to_continue', 'mcodelti', (object)array('link' => $redirect));
    $html = <<< EOD
<html>
<head>
...
EOD;
    ...
    if (!empty($id)) {
        ...
        switch($toolproxy->state) {
            case mcodelti_TOOL_PROXY_STATE_ACCEPTED:
                $redirect->param('tab', 'tp_accepted');
                break;
            case mcodelti_TOOL_PROXY_STATE_REJECTED:
                $redirect->param('tab', 'tp_rejected');
                break;
            case mcodelti_TOOL_PROXY_STATE_PENDING:
