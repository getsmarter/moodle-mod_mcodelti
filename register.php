...
$redirect = new moodle_url('/mod/mcodelti/toolproxies.php', array('tab' => $tab));
...
foreach ($toolproxies as $key => $toolproxy) {
    if (($toolproxy->state == mcodelti_TOOL_PROXY_STATE_PENDING) || ($toolproxy->state == mcodelti_TOOL_PROXY_STATE_ACCEPTED)) {
...
$redirect = new moodle_url('/mod/mcodelti/toolproxies.php');
if ($duplicate) {
    redirect($redirect,  get_string('duplicateregurl', 'mcodelti'));
}


$profileservice = mcodelti_get_service_by_name('profile');
if (empty($profileservice)) {
    redirect($redirect,  get_string('noprofileservice', 'mcodelti'));
}

$url = new moodle_url('/mod/mcodelti/register.php', array('id' => $id));
$PAGE->set_url($url);

admin_externalpage_setup('mcodeltitoolproxies');


$PAGE->set_heading(get_string('toolproxyregistration', 'mcodelti'));
$PAGE->set_title("{$SITE->shortname}: " . get_string('toolproxyregistration', 'mcodelti'));

// Print the page header.
echo $OUTPUT->header();

echo $OUTPUT->heading(get_string('toolproxyregistration', 'mcodelti'));

echo $OUTPUT->box_start('generalbox');

// Request the registration request content with an object tag.
$registration = new moodle_url('/mod/mcodelti/registration.php',
...
echo get_string('register_warning', 'mcodelti');
echo "\n</p>\n";
...
$resize = '
        ...
        //<![CDATA[
            ...
            function doOnload() {
                window.clearTimeout(mod_mcodelti_timer);
            }
            var mod_mcodelti_timer = window.setTimeout(doReveal, 20000);
