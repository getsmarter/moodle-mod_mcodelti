 ...
 *
 * It is used to create a new form used to pre-configure mcodelti activities
 *
...
require_once($CFG->dirroot.'/mod/mcodelti/edit_form.php');
...
if ($returnto == 'toolconfigure') {
    $returnurl = new moodle_url($CFG->wwwroot . '/mod/mcodelti/toolconfigure.php');
}
...
if (!empty($id)) {
    $type = mcodelti_get_type_type_config($id);
    if (!empty($type->toolproxyid)) {
        ...
        $redirect = new moodle_url('/mod/mcodelti/toolssettings.php', $params);
        redirect($redirect);
    }
} else {
    $type = new stdClass();
    // Assign a default empty value for the mcodelti_icon.
    $type->mcodelti_icon = '';
    $type->mcodelti_secureicon = '';
}

$pageurl = new moodle_url('/mod/mcodelti/typessettings.php');
if (!empty($id)) {
    $pageurl->param('id', $id);
}
if (!empty($returnto)) {
...
$redirect = "$CFG->wwwroot/$CFG->admin/settings.php?section=modsettingmcodelti&tab={$tab}";
if (!empty($returnurl)) {
    $redirect = $returnurl;
}

if ($action == 'accept') {
    mcodelti_set_state_for_type($id, mcodelti_TOOL_STATE_CONFIGURED);
    redirect($redirect);
} else if ($action == 'reject') {
    mcodelti_set_state_for_type($id, mcodelti_TOOL_STATE_REJECTED);
    redirect($redirect);
...
if (mcodelti_request_is_using_ssl() && !empty($type->mcodelti_secureicon)) {
    $type->oldicon = $type->mcodelti_secureicon;
} else {
    $type->oldicon = $type->mcodelti_icon;
}

$form = new mod_mcodelti_edit_types_form($pageurl, (object)array('isadmin' => true, 'istool' => false));

if ($data = $form->get_data()) {
    $type = new stdClass();
    if (!empty($id)) {
        $type->id = $id;
        mcodelti_load_type_if_cartridge($data);
        mcodelti_update_type($type, $data);
...
$PAGE->set_title("$SITE->shortname: " . get_string('toolsetup', 'mcodelti'));
$PAGE->navbar->add(get_string('mcodelti_administration', 'mcodelti'), $CFG->wwwroot.'/'.$CFG->admin.'/settings.php?section=modsettingmcodelti');

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('toolsetup', 'mcodelti'));
