...
(function(){
    ...
    M.mod_mcodelti = M.mod_mcodelti || {};

    M.mod_mcodelti.mcodelti_SETTING_NEVER = 0;
    M.mod_mcodelti.mcodelti_SETTING_ALWAYS = 1;
    M.mod_mcodelti.mcodelti_SETTING_DELEGATE = 2;

    M.mod_mcodelti.editor = {
        init: function(yui3, settings){
            ...
            contentItemButton.on('click', function() {
                ...
                if (contentItemId) {
                    ...
                    require(['mod_mcodelti/contentitem'], function(contentitem) {
        ...
        updateAutomaticToolMatch: function(field){
            ...
            var id = field.get('id') + '_mcodelti_automatch_tool';
            ...
            if(selectedToolType > 0){
                ...
                if(match && match[1] && match[1].toLowerCase() === selectedOption.getAttribute('domain').toLowerCase()){
                    automatchToolDisplay.set('innerHTML',  '<img style="vertical-align:text-bottom" src="' + self.settings.green_check_icon_url + '" />' + M.util.get_string('using_tool_configuration', 'mcodelti') + selectedOption.get('text'));
                } else {
                    // The entered URL does not match the domain of the tool configuration
                    automatchToolDisplay.set('innerHTML', '<img style="vertical-align:text-bottom" src="' + self.settings.warning_icon_url + '" />' + M.util.get_string('domain_mismatch', 'mcodelti'));
                }
            }
            ...
            if(key.get('value') !== '' && secret.get('value') !== ''){
                automatchToolDisplay.set('innerHTML',  '<img style="vertical-align:text-bottom" src="' + self.settings.green_check_icon_url + '" />' + M.util.get_string('custom_config', 'mcodelti'));
            }
            ...
            var continuation = function(toolInfo, inputfield){
                ...
                if(toolInfo.toolname){
                    automatchToolDisplay.set('innerHTML',  '<img style="vertical-align:text-bottom" src="' + self.settings.green_check_icon_url + '" />' + M.util.get_string('using_tool_configuration', 'mcodelti') + toolInfo.toolname);
                } else if(!selectedToolType) {
                    // Inform them custom configuration is in use
                    if(key.get('value') === '' || secret.get('value') === ''){
                        automatchToolDisplay.set('innerHTML', '<img style="vertical-align:text-bottom" src="' + self.settings.warning_icon_url + '" />' + M.util.get_string('tool_config_not_found', 'mcodelti'));
                    }
                }
                if (toolInfo.cartridge) {
                    automatchToolDisplay.set('innerHTML', '<img style="vertical-align:text-bottom" src="' + self.settings.green_check_icon_url +
                                             '" />' + M.util.get_string('using_tool_cartridge', 'mcodelti'));
                }
            };
        ...
        updatePrivacySettings: function(toolInfo){
            if(!toolInfo || !toolInfo.toolid){
                toolInfo = {
                    sendname: M.mod_mcodelti.mcodelti_SETTING_DELEGATE,
                    sendemailaddr: M.mod_mcodelti.mcodelti_SETTING_DELEGATE,
                    acceptgrades: M.mod_mcodelti.mcodelti_SETTING_DELEGATE
                }
            }
            ...
            for(setting in privacyControls){
                if(privacyControls.hasOwnProperty(setting)){
                    ...
                    if(settingValue == M.mod_mcodelti.mcodelti_SETTING_NEVER){
                        control.set('disabled', true);
                        control.set('checked', false);
                        control.set('title', M.util.get_string('forced_help', 'mcodelti'));
                    } else if(settingValue == M.mod_mcodelti.mcodelti_SETTING_ALWAYS){
                        control.set('disabled', true);
                        control.set('checked', true);
                        control.set('title', M.util.get_string('forced_help', 'mcodelti'));
                    } else if(settingValue == M.mod_mcodelti.mcodelti_SETTING_DELEGATE){
        ...
        addOptGroups: function(){
            ...
            if(typeSelector.one('option[courseTool=1]')){
                ...
                var globalGroup = Y.Node.create('<optgroup />')
                                    ...
                                    .set('label', M.util.get_string('global_tool_types', 'mcodelti'));

                var courseGroup = Y.Node.create('<optgroup />')
                                    .set('id', 'course_tool_group')
                                    .set('label', M.util.get_string('course_tool_types', 'mcodelti'));
        ...
        createTypeEditorButtons: function(){
            ...
            var createIcon = function(id, toomcodeltip, iconUrl){
                return Y.Node.create('<a />')
                        .set('id', id)
                        .set('title', toomcodeltip)
                        .setStyle('margin-left', '.5em')
                        .set('href', 'javascript:void(0);')
                        .append(Y.Node.create('<img src="' + iconUrl + '" />'));
            }

            var addIcon = createIcon('mcodelti_add_tool_type', M.util.get_string('addtype', 'mcodelti'), this.settings.add_icon_url);
            var editIcon = createIcon('mcodelti_edit_tool_type', M.util.get_string('edittype', 'mcodelti'), this.settings.edit_icon_url);
            var deleteIcon  = createIcon('mcodelti_delete_tool_type', M.util.get_string('deletetype', 'mcodelti'), this.settings.delete_icon_url);
            ...
            editIcon.on('click', function(e){
                ...
                if(self.getSelectedToolTypeOption().getAttribute('editable')){
                    ...
                    alert(M.util.get_string('cannot_edit', 'mcodelti'));
                }
            });
            ...
            deleteIcon.on('click', function(e){
                ...
                if(self.getSelectedToolTypeOption().getAttribute('editable')){
                    if(confirm(M.util.get_string('delete_confirmation', 'mcodelti'))){
                        self.deleteTool(toolTypeId);
                    }
                } else {
                    alert(M.util.get_string('cannot_delete', 'mcodelti'));
                }
            });
        ...
        toggleEditButtons: function(){
            var mcodelti_edit_tool_type = Y.one('#mcodelti_edit_tool_type');
            var mcodelti_delete_tool_type = Y.one('#mcodelti_delete_tool_type');
            ...
            if(this.getSelectedToolTypeOption().getAttribute('editable')){
                mcodelti_edit_tool_type.setStyle('opacity', '1');
                mcodelti_delete_tool_type.setStyle('opacity', '1');
            } else {
                mcodelti_edit_tool_type.setStyle('opacity', '.2');
                mcodelti_delete_tool_type.setStyle('opacity', '.2');
            }
        },
        ...
        addToolType: function(toolType){
            ...
            require(["core/notification"], function (notification) {
                notification.addNotification({
                    message: M.util.get_string('tooltypeadded', 'mcodelti'),
                    type: "success"
                });
            });
        },
        ...
        updateToolType: function(toolType){
            ...
            require(["core/notification"], function (notification) {
                notification.addNotification({
                    message: M.util.get_string('tooltypeupdated', 'mcodelti'),
                    type: "success"
                });
            });
        },
            ...
            Y.io(self.settings.instructor_tool_type_edit_url + '&action=delete&typeid=' + toolTypeId, {
                on: {
                    success: function(){
                        ...
                        require(["core/notification"], function (notification) {
                            notification.addNotification({
                                message: M.util.get_string('tooltypedeleted', 'mcodelti'),
                                type: "success"
                            });
                        });
                    },
                    failure: function(){
                        require(["core/notification"], function (notification) {
                            notification.addNotification({
                                message: M.util.get_string('tooltypenotdeleted', 'mcodelti'),
                                type: "problem"
                            });
                        });
                    }
        ...
        /**
         * Resets the values of fields related to the mcodelti tool settings.
         */
