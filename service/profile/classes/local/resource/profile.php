 ...
 *
 * @package    mcodeltiservice_profile
...
namespace mcodeltiservice_profile\local\resource;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class profile extends \mod_mcodelti\local\mcodeltiservice\resource_base {

    /**
     * Class constructor.
     *
     * @param mcodeltiservice_profile\local\resource\profile $service Service instance
     */
    public function __construct($service) {
        ...
        $this->formats[] = 'application/vnd.ims.mcodelti.v2.toolconsumerprofile+json';
        $this->methods[] = 'GET';
    ...
    public function execute($response) {
        ...
        $version = service_base::mcodelti_VERSION2P0;
        $params = $this->parse_template();
        $ok = $this->get_service()->check_tool_proxy($params['tool_proxy_id']);
        if (!$ok) {
            $response->set_code(404);
        } else if (optional_param('mcodelti_version', '', PARAM_ALPHANUMEXT) != $version) {
            ...
            $serviceoffered = '';
            $sep = '';
            $services = \core_component::get_plugin_list('mcodeltiservice');
...
EOD;
            ...
            if (!empty($CFG->mod_mcodelti_institution_name)) {
                $consumername = $CFG->mod_mcodelti_institution_name;
                $consumerdesc = '';
            } else {
...
{
  "@context":[
    "http://purl.imsglobal.org/ctx/mcodelti/v2/ToolConsumerProfile",
    {
      "tcp":"{$id}#"
    }
  ],
...
EOD;
    ...
    public function get_endpoint() {
        ...
        return parent::get_endpoint() . '?mcodelti_version=' . service_base::mcodelti_VERSION2P0;
