 ...
 *
 * @package    mcodeltiservice_toolproxy
...
namespace mcodeltiservice_toolproxy\local\resource;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class toolproxy extends \mod_mcodelti\local\mcodeltiservice\resource_base {

    /**
     * Class constructor.
     *
     * @param mcodeltiservice_toolproxy\local\resource\toolproxy $service Service instance
     */
    public function __construct($service) {
        ...
        $this->formats[] = 'application/vnd.ims.mcodelti.v2.toolproxy+json';
        $this->methods[] = 'POST';
    ...
    public function execute($response) {
        ...
        if ($ok && isset($toolproxyjson->security_contract->tool_service)) {
            $contexts = mcodelti_get_contexts($toolproxyjson);
            ...
            foreach ($tpservices as $service) {
                $fqid = mcodelti_get_fqid($contexts, $service->service);
                if (substr($fqid, 0, strlen($context)) !== $context) {
                    $errors[] = $service->service;
                } else {
                    $id = explode('#', $fqid, 2);
                    $aservice = mcodelti_get_service_by_resource_id($services, $id[1]);
            ...
            foreach ($resources as $resource) {
                ...
                foreach ($resource->message as $message) {
                    if (in_array($message->message_type, $messagetypes)) {
                        $launchable = $launchable || ($message->message_type === 'basic-mcodelti-launch-request');
        ...
        if ($ok) {
            ...
            foreach ($tools as $tool) {
                ...
                $launchrequest = $messages['basic-mcodelti-launch-request'];
                $config = new \stdClass();
                $config->mcodelti_toolurl = "{$baseurl}{$launchrequest->path}";
                $config->mcodelti_typename = $tool->name;
                $config->mcodelti_coursevisible = 1;
                ...
                if (isset($messages['ContentItemSelectionRequest'])) {
                    ...
                    $config->mcodelti_contentitem = 1;
                    if ($launchrequest->path !== $contentitemrequest->path) {
                        $config->mcodelti_toolurl_ContentItemSelectionRequest = $baseurl . $contentitemrequest->path;
                    }
                    $contentitemcapabilities = implode("\n", $contentitemrequest->enabled_capability);
                    $config->mcodelti_enabledcapability_ContentItemSelectionRequest = $contentitemcapabilities;
                    $contentitemparams = self::mcodelti_extract_parameters($contentitemrequest->parameter);
                    $config->mcodelti_parameter_ContentItemSelectionRequest = $contentitemparams;
                }
                ...
                $type->parameter = self::mcodelti_extract_parameters($launchrequest->parameter);
                ...
                $ok = $ok && (mcodelti_add_type($type, $config) !== false);
            }
            if (isset($toolproxyjson->custom)) {
                mcodelti_set_tool_settings($toolproxyjson->custom, $toolproxy->id);
            }
        }
        ...
        if (!empty($toolproxy)) {
            if ($ok) {
                ...
                $toolproxy->state = mcodelti_TOOL_PROXY_STATE_ACCEPTED;
                ...
                $body = <<< EOD
{
  "@context" : "http://purl.imsglobal.org/ctx/mcodelti/v2/ToolProxyId",
  "@type" : "ToolProxy",
  "@id" : "{$url}",
...
EOD;
                ...
                $response->set_content_type('application/vnd.ims.mcodelti.v2.toolproxy.id+json');
                $response->set_body($body);
            } else {
                // Otherwise reject the tool proxy.
                $toolproxy->state = mcodelti_TOOL_PROXY_STATE_REJECTED;
     ...
     */
    private static function mcodelti_extract_parameters($parameters) {
