 ...
 *
 * @package    mcodeltiservice_memberships
...
namespace mcodeltiservice_memberships\local\service;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class memberships extends \mod_mcodelti\local\mcodeltiservice\service_base {
    ...
    public function __construct() {
        ...
        parent::__construct();
        $this->id = 'memberships';
        $this->name = get_string('servicename', 'mcodeltiservice_memberships');
    ...
    public function get_resources() {
        ...
        if (empty($this->resources)) {
            ...
            $this->resources[] = new \mcodeltiservice_memberships\local\resource\contextmemberships($this);
            $this->resources[] = new \mcodeltiservice_memberships\local\resource\linkmemberships($this);
        }
     ...
     *
     * @param \mod_mcodelti\local\mcodeltiservice\resource_base $resource       Resource handling the request
     ...
     * @param object            $mcodelti        mcodelti instance record
     * @param info_module       $info       Conditional availability information for mcodelti instance (null if context-level request)
     *
     * @return array
     */
    public static function get_users_json($resource, $context, $id, $tool, $role, $limitfrom, $limitnum, $mcodelti, $info) {
        ...
        $json = self::users_to_json($resource, $users, $id, $tool, $exclude, $limitfrom, $limitnum, $mcodelti, $info);
     ...
     * @param int    $limitnum            Maximum number of records to be returned
     * @param object $mcodelti                 mcodelti instance record
     * @param \core_availability\info_module  $info     Conditional availability information for mcodelti instance
     *
     * @return string
     */
...
EOD;
        ...
        foreach ($users as $user) {
            ...
            if ($include) {
                ...
                if (in_array('Result.sourcedId', $enabledcapabilities) && !empty($mcodelti) && !empty($mcodelti->servicesalt)) {
                    $member->resultSourcedId = json_encode(mcodelti_build_sourcedid($mcodelti->id, $user->id, $mcodelti->servicesalt,
                                                           $mcodelti->typeid));
                }
                $roles = explode(',', mcodelti_get_ims_role($user->id, null, $id, true));
