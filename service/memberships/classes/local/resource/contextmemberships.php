 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class contextmemberships extends \mod_mcodelti\local\mcodeltiservice\resource_base {
    ...
    public function execute($response) {
        ...
        try {
            ...
            if (!($tool = $DB->get_record('mcodelti_types', array('id' => $params['tool_code']),
                                    'toolproxyid,enabledcapability,parameter', IGNORE_MISSING))) {
                throw new \Exception(null, 404);
            }
            $toolproxy = $DB->get_record('mcodelti_tool_proxies', array('id' => $tool->toolproxyid), 'guid', IGNORE_MISSING);
    ...
    public function parse_value($value) {
        ...
        if (!empty($id)) {
            $cm = get_coursemodule_from_id('mcodelti', $id, 0, false, IGNORE_MISSING);
            $mcodelti = $DB->get_record('mcodelti', array('id' => $cm->instance), 'typeid', IGNORE_MISSING);
            if ($mcodelti && !empty($mcodelti->typeid)) {
                $this->params['tool_code'] = $mcodelti->typeid;
            }
        }
