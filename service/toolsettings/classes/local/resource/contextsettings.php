 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class contextsettings extends \mod_mcodelti\local\mcodeltiservice\resource_base {

    /**
     * Class constructor.
     *
     * @param mcodeltiservice_toolsettings\local\resource\contextsettings $service Service instance
     */
    ...
    public function execute($response) {
        ...
        if (!$ok) {
            ...
            $systemsetting = null;
            $contextsettings = mcodelti_get_tool_settings($this->get_service()->get_tool_proxy()->id, $contextid);
            if (!empty($bubble)) {
            ...
            if ($response->get_request_method() == 'GET') {
                ...
                if ($ok) {
                    mcodelti_set_tool_settings($settings, $this->get_service()->get_tool_proxy()->id, $contextid);
                } else {
